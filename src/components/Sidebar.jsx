  ...
  useEffect(() => {
    ...
    const fetchIncidents = async () => {
      try {
        ...
        const res = await axios.get(`${BASE_API}/incidents`, {
          headers,
          validateStatus: () => true,
        });
        if (!mounted) return;
    ...
    <>
      ...
      {open && (
        <>
          ...
          />
          <aside className="md:hidden fixed top-0 left-0 z-[65] w-4/5 max-w-xs h-screen p-4 pt-6 bg-gray-900 text-white shadow-lg">
      ...
      {/* ===== Sidebar desktop (toujours visible et FIXE) ===== */}
      <aside className="hidden md:flex md:flex-col md:fixed md:top-[56px] md:left-0 md:z-40 md:w-64 md:h-[calc(100vh-56px)] md:p-4 md:pt-6 md:bg-gray-900 md:text-white md:shadow md:overflow-y-auto">
        <Links onClick={undefined} />

        {/* Carte signature bas (desktop) */}
        <a
          href="https://www.facebook.com/mohamedsoighir.attoumani"
          target="_blank"
          rel="noopener noreferrer"
